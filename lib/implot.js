(function (factory) {
    ...
    ImPlotDefault.prototype.horizontal_bar_series = function (config, x, y, w, h, series) {
        ...
        if (series.hasOwnProperty('offset')) {
            ...
            console.log(series.offset);

            bar_w = (w / series.length) - config.gap;

            for (i = 0; i < series.data.length; i++) {
                bar_x = x + (i - offset) * (bar_w + config.gap) + config.gap * 0.5;
                this.horizontal_bar(config, bar_x, y + h, bar_w, scale_y, series.data[i]);
            }
        } else {
            bar_w = (w / series.data.length) - config.gap;

            for (i = 0; i < series.data.length; i++) {
                bar_x = x + i * (bar_w + config.gap) + config.gap * 0.5;
                this.horizontal_bar(config, bar_x, y + h, bar_w, scale_y, series.data[i]);
            }
        }
    };
    ...
    ImPlotDefault.prototype.line_series = function (config, x, y, w, h, series) {
        ...
        var interval = w / series.data.length;
        var scale_y = h / config.maximum;

        var last_pos = {x: x + 0.5 * interval, y: y + h - series.data[0] * scale_y};
        ...
        for (var i = 1; i < series.data.length; i++) {
            var pos = {x: x + (i + 0.5) * interval, y: y + h - series.data[i] * scale_y};
    ...
    ImPlotDefault.prototype.pie_chart = function (config, series, center_pos, radius) {
        config = _.defaults(config, {
            parts: [],
            gap: 0
        });
    ...
    ImPlotDefault.prototype.pie_ring_chart = function (config, series, center_pos, radius) {
        config = _.defaults(config, {
            outer_radius: radius,
    ...
    ImPlotDataQueue.prototype._interpolate = function (a, b) {
        ...
        if (a.data[0].hasOwnProperty('offset')) {
            for (series_index = 0; series_index < a.data.length; series_index++) {
                ...
                result.push({data: series_result, offset: series_a.offset * factor_a + series_b.offset * factor_b, length: series_b.data.length});
            }
        } else {
            for (series_index = 0; series_index < a.data.length; series_index++) {
                series_a = a.data[series_index];
