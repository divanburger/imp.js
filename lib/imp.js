(function (factory) {
    ...
    var Color = {
        ...
        generate_color_from_palette: function (palette, x) {
            return Color.rgb_string_to_srgb(palette[x % palette.length]);
        },

        generate_color: function (x, max_x, h, hr) {
            var l = x * 1.0 / max_x;
            var lch = new Color.LCHColor(l * 200.0 - l * l * 100.0,
                                         (l - Math.pow(l, 4.0)) * 120.0,
                                         h + l * hr);
            return Color.lch_to_srgb(lch);
        }
    };
    ...
    ImpDataQueue.prototype.update = function (delta) {
        if (delta > 1000.0) {
            ...
            console.log("Browser paused : " + delta);
    ...
    Imp.prototype._on_resize = function () {
        ...
        this.canvas.canvas.width = this.size.x;
        this.canvas.canvas.height = this.size.y;
    };
